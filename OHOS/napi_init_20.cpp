...
bool IS_CREATED_SURFACE=false;

void MyLog(const std::string &str)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x1, "MyLog", "%{public}s", str.c_str());
}
...
void OnArkUIXComponentFrameCallback(ArkUI_NodeHandle node, uint64_t timestamp, uint64_t targetTimestamp)
{
    ...
    if(IS_CREATED_SURFACE)
    {
        //VULKAN_TRIANGLE.Draw(timestamp * 0.001);
        VULKAN_TRIANGLE.Draw(timestamp*1e-9);
    }
}
...
void OnSurfaceChanged(OH_ArkUI_SurfaceHolder *holder, uint64_t width, uint64_t height)
{
    ...
    std::string log_str=std::string("OnSurfaceChanged (with = ")+width_str+std::string(", height = ")+height_str+std::string(")");
    MyLog(log_str);
    
    if(!IS_CREATED_SURFACE)
    {
        auto native_window = OH_ArkUI_XComponent_GetNativeWindow(holder);
        VULKAN_TRIANGLE.CreateSurface(native_window,width,height);
        IS_CREATED_SURFACE=true;
    }
}
...
{
    // TODO: Init Vulkan
    // MyLog("Begin VulkanTriangle* VULKAN_TRIANGLE=new VulkanTriangle();");
    //VulkanTriangle* VULKAN_TRIANGLE=new VulkanTriangle();
    //MyLog("After VulkanTriangle* VULKAN_TRIANGLE=new VulkanTriangle();");
}
